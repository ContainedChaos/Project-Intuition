<?php
  ...
  function sendMail($email,$vkey)
  {
    ...
    $mail->Body = "Thank you for registering. Click the link to verify your email
                   <a href='http://localhost/Project-Intuition/verifystudent.php?email=$email&vkey=$vkey'> Verify </a>";
  ...
  if(!empty($name) && !empty($gender) && !empty($grade) && !empty($version) && !empty($email) && !empty($password) && !empty($confirm))
  {
    if($password == $confirm)
    {
      ...
      else
      {
        ...
        if (strlen($_POST['phone']) != 11 strlen($_POST['phone']) == 0 || !preg_match("/[0-9]/", $_POST['phone'])) {
